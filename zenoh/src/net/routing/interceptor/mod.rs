...
use self::{accesscontrol::AclEnforcer, authz::NewPolicyEnforcer};
...
pub(crate) fn interceptor_factories(config: &Config) -> Vec<InterceptorFactory> {
    ...
    */
    ...
    /* if config condition is selected this will be initialised; putting true for now */
    ...
    if acl_enabled {
        let mut policy_enforcer = NewPolicyEnforcer::new();
