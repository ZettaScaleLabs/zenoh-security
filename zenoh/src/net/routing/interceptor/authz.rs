...
//policy deisgn for each subject

//#[derive(Clone, Debug, Eq, Hash, PartialEq)]
//struct KeTreeFast(KeBoxTree<i32>);
...
//can also have a mapping for subject to index such that only index is matched

type SubjectTreeMap = Vec<FxHashMap<Permission, KeTreeRule>>;

// //type Subject = String; //can change later
...
pub struct Request {
    pub(crate) sub: Subject, //removed String
        ...
        if self.acl_enabled {
            match acl_config.policy_list {
                Some(policy_list) => {
                    ...
                    //get the policy list from the list of stuff we have
                    // let subject_list: Vec<Subject> = vec![
                    //     Subject::NetworkInterface("wifi0".to_string()),
                    //     //Subject::NetworkInterface("wlan1".to_string()),
                    // ];
